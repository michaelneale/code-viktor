...
def scan_repository(repo_path):
    ...
    for root, dirs, files in os.walk(repo_path):
        for file in files:
            if file.endswith(('.py', '.java', '.js', '.cpp', '.c', '.h', '.rb', '.go', '.rs', '.php', '.html', '.css', '.md')):
...
def main():
    ...
    parser.add_argument('db_path', type=str, help='Path to save the vector database')
    args = parser.parse_args()

    file_paths, file_contents = scan_repository(args.repo_path)
    embeddings = build_vector_database(file_contents)
    save_vector_database(file_paths, embeddings, args.db_path)
